...
class PPOTrainer(BaseRLTrainer):
    ...
    def _eval_checkpoint(
        ...
        if (
            ...
            # self._agent.load_state_dict(ckpt_dict) # Original
            # Below: remove actor_critic. from state_dict because it mismatches to current config if loading pretrained model from data/habitat_baselines_v2
            new_state_dict = {'state_dict': {k[len("actor_critic.") :]: v for k, v in ckpt_dict['state_dict'].items() if "actor_critic." in k}}
            self._agent.load_state_dict(new_state_dict)
